...
pub(crate) struct NetstackRunner {
    ...
    /// Scratch buffer reused each poll cycle for accepted sessions.
    scratch_accepted: Vec<Session>,
    /// Scratch buffer reused each poll cycle for egress packets.
    scratch_egress: Vec<EgressPacket>,
}
...
impl NetstackRunner {
    pub(crate) fn new(
        ...
        Self {
            ...
            egress_tx,
            accept_tx,
            scratch_accepted: Vec::new(),
            scratch_egress: Vec::new(),
        }
    }
    ...
    async fn poll_once(&mut self) {
        let now = Instant::now();
        self.scratch_accepted.clear();
        self.scratch_egress.clear();

        let shared = self.shared.clone();
        let log_level = shared.core.lock().unwrap().config.log_level;

        {
            ...
            Self::pre_pass(&shared, &mut core, now, &mut self.scratch_accepted);
            ...
            Self::post_pass(&shared, &mut core, now, &mut self.scratch_accepted);

            // 4. Collect packets generated by the stack.
            for slot in &mut core.slots {
                for data in slot.device.egress.drain(..) {
                    self.scratch_egress.push(EgressPacket {
        ...
        for pkt in self.scratch_egress.drain(..) {
        ...
        for session in self.scratch_accepted.drain(..) {
    ...
    fn pre_pass(shared: &Arc<Shared>, core: &mut Core, now: Instant, accepted: &mut Vec<Session>) {
        let max_tcp = core.config.tcp_max_connections;
        let max_udp = core.config.udp_max_sessions;

        for i in 0..core.slots.len() {
            let mut packet_sets = std::mem::take(&mut core.slots[i].device.ingress);

            for pkt in &packet_sets {
                ...
                match l4 {
                    ...
                    L4::Udp => {
                        ...
                        if let Some(session) = Self::alloc_udp(shared, core, i, key, now) {
            ...
            core.slots[i].device.ingress.append(&mut packet_sets);
        }
    }

    /// Create a listening TCP socket for a new SYN and register the session.
    fn alloc_tcp(core: &mut Core, i: usize, key: SessionKey) {
        ...
        let mut socket = match core.slots[i].tcp_pool.pop_front() {
            Some(s) => s,
            None => {
    ...
    fn post_pass(shared: &Arc<Shared>, core: &mut Core, now: Instant, accepted: &mut Vec<Session>) {
        ...
        for key in tcp_reap {
                ...
                && let Some(slot) = core.slots.iter_mut().find(|s| s.id == key.interface)
            {
                ...
                let removed = slot.sockets.remove(entry.handle);
                // Keep the socket warm for reuse only while the pool is under
                // its configured cap; otherwise drop it so the buffers are
                // freed (lower resident RSS at the cost of re-allocation).
                if let smoltcp::socket::Socket::Tcp(s) = removed
                    && slot.tcp_pool.len() < slot.tcp_pool_max()
                {
                    slot.tcp_pool.push_back(s);
                }
